...
engine = create_engine(
    # For cockroach demo:
    #'cockroachdb://<username>:<password>@<hostname>:<port>/blox?sslmode=require',   #info is stored in env, obviously wont share my codes
    # For CockroachCloud:
    'cockroachdb://'+env.username+':'+env.password+'@free-tier.gcp-us-central1.cockroachlabs.cloud:26257/' + 'able-moose'
+'.bank?sslmode=verify-full&sslrootcert=cockroachbase/<ca.crt>',
...
def create_true_account(sess, num)
    account = {"privatekey":["[publickeys]","[cryptocurrency]"]}
    new = {env.private:[env.publickeys, env.values]}
    sess.add_all(new)


run_transaction(sessionmaker(bind=engine),
                lambda s: create_true_account(s))


def get_private_account_keys():
    """ Helper function for getting random existing account IDs.
    """
    ids = (tuple(seen_account_ids))
    id = ids[env.number]
    return id
